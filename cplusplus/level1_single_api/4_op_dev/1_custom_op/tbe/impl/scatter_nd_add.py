...
class Scatter():
    ...
    def calc_process(self, mask, dest_addr, src_addr1, src_addr2, repeat_times,
        ...
        if self.compute_type == "vadd":
            ...
            self.tik_instance.vsub(mask, dst_ub, src1_ub, src2_ub, repeat_times,
                                   1, 1, 1, compute_repeat_strid,
                                   compute_repeat_strid, compute_repeat_strid)
            ...
            if tbe_platform.cce_conf.api_check_support("tik.vdiv", "float32"):
                ...
                self.tik_instance.vrec(mask, tmp_tensor, src2_ub, repeat_times,
                                       ...
                                       compute_repeat_strid)
                self.tik_instance.vmul(mask, src2_ub, src2_ub, tmp_tensor,
                                       repeat_times, 1, 1, 1,
                                       compute_repeat_strid,
                                       compute_repeat_strid,
                                       compute_repeat_strid)
                self.tik_instance.vadds(mask, src2_ub, src2_ub, NEG_TWO,
                                        repeat_times, 1, 1,
                                        compute_repeat_strid,
                                        compute_repeat_strid)
                self.tik_instance.vmul(mask, src2_ub, src2_ub, tmp_tensor,
                                       ...
                                       compute_repeat_strid,
                                       compute_repeat_strid)
                self.tik_instance.vmuls(mask, src2_ub, src2_ub, NEG_ONE,
                                        repeat_times, 1, 1,
                                        compute_repeat_strid,
                                        compute_repeat_strid)
                # src1_ub * (1/src2_ub)
            ...
            self.tik_instance.vmul(mask, dst_ub, src1_ub, src2_ub, repeat_times,
                                   1, 1, 1, compute_repeat_strid,
                                   compute_repeat_strid, compute_repeat_strid)
        elif self.compute_type == "update":
            self.tik_instance.vmuls(mask, dst_ub, src1_ub, 0, repeat_times, 1,
                                    1, compute_repeat_strid,
                                    compute_repeat_strid)
            self.tik_instance.vadd(mask, dst_ub, src1_ub, src2_ub, repeat_times,
                                   1, 1, 1, compute_repeat_strid,
                                   compute_repeat_strid, compute_repeat_strid)
