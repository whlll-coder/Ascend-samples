...
function downloadData() {
    ...
    wget -O ${project_path}/data/"video2.mp4"  ${data_source}/video1.mp4  --no-check-certificate
    if [ $? -ne 0 ];then
        echo "download test1.jpg failed, please check Network."
        return 1
    fi

    wget -O ${project_path}/data/"video3.mp4"  ${data_source}/video1.mp4  --no-check-certificate
    if [ $? -ne 0 ];then
        echo "download test1.jpg failed, please check Network."
        return 1
    fi
    wget -O ${project_path}/data/"video4.mp4"  ${data_source}/video1.mp4  --no-check-certificate
    if [ $? -ne 0 ];then
        echo "download test1.jpg failed, please check Network."
        return 1
    fi
