...
def main_worker(gpu_index, gpu_num, args):
    ...
    amct.create_quant_retrain_config(
        config_file, model, get_input_data([(1, 3, SIZE, SIZE)], model), args.config_defination)
    ...
    model = amct.create_quant_retrain_model(
        config_file, model, record_file, get_input_data([(1, 3, SIZE, SIZE)], model))
    ...
    if not args.distributed or (args.distributed and gpu_index == 0):
        quant_top1, quant_top5 = cal_quant_model_accuracy(model, gpu_index, val_loader, args, config_file, record_file)

        print('[INFO] ResNet-101 before retrain top1:{:.2f}% top5:{:.2f}%'.format(ori_top1, ori_top5))
        print('[INFO] ResNet-101 after retrain top1:{:.2f}% top5:{:.2f}%'.format(quant_top1, quant_top5))
